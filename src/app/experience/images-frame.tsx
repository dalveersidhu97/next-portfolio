    ...
    return <>
        ...
        <div className="flex gap-2 justify-center">
            {images.map((_, i) => <Fragment key={`scrollnob-${i}`}>
                <button onClick={(e) => {e.stopPropagation();e.preventDefault();scrollToIndex(i)}} className={`transition-all w-2.5 h-2.5 ${i===activeIndex?'bg-primary border-primary dark:bg-foregroundDark dark:border-foregroundDark':'bg-inherit'} border rounded-full`}></button>
            </Fragment>)}
        </div>
    </>;
}